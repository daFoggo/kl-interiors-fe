...
export function DataTableFilterList<TData>({
  ...
  const updateFilter = React.useCallback(
    (
      ...
      // Use direct filters array to avoid stale state in functional updates
  ...
  const onFilterAdd = React.useCallback(() => {
    ...
    // Use immediate update for adding (or debounced is fine too, but add needs immediate feedback)
    const newFilter = {
  ...
  // Sort filters based on column definition if needed?
  // Since this is a Sortable list, we probably shouldn't auto-sort continuously as it fights drag-drop.
  // We leave filters order as is (managed by Sortable).
...
function onFilterInputRender<TData>({
  ...
  switch (filter.variant) {
    ...
    case "range": {
      if (
        ...
        return (
          <DataTableRangeFilter
            ...
            column={column}
            inputId={inputId}
            onFilterUpdate={onDebouncedFilterUpdate}
          />
        );
      }
      ...
      return (
        <InputGroup className="h-8 w-full rounded">
          <InputGroupInput
            id={inputId}
